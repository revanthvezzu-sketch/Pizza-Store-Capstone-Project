<h1>Pizza Store Capstone Project<h1>

<h2>Features</h2>

<p>
  <i>In workflow-</i> <br>
  1. User login/register<br>
  2. User Order the item<br>
  3. User Item store in card<br>
  4. User place order<br>
  5. User's Order store in admin, admin will edit item<br>
  6. Admin will approve/deny the order<br>
  7. Admin approve/deny, bill will genrated<br>
  8. Admin can change status of the order(Ready, Deliverd, cancalled)<br>
  9. Bill will be popup in both admin and user<br>
  10. Bill will be stored in user and admin<br>
 <p>

<p>

<h4>Extra Features</h4>

<i>In workflow-</i><br>
1.Timer in order (user)<br>
2.bill aprove or deny (admin)<br>
3.bill popup notification (admin and user)<br>
4.change the status (admin) <br>
5.revenue analytics dashboard (admin)<br>
6.change or edit items (admin) <br>

<h4>Inside frontend code:</h4>

1.<i>Items image<i> - Array <br>
(solving - out of running images )

 2.<i>In Timer<i>it has 2 timers - 
    • Show timer (in ui)<br>
    • Actual timer (in ux)<br>
    Both timer are same but the thing is what client<br>
    see that ui timer and inside the ui timer have ux timer.
</p>

Page
<img src="https://github.com/revanthvezzu-sketch/Pizza-Store-Capstone-Project/blob/9e40c7dc4ad5b5e9f85cb434a14b7826e954db5b/Readme%20and%20Img/Images/img1.png" alt="Page">

RegisterPage
<img src="https://github.com/revanthvezzu-sketch/Pizza-Store-Capstone-Project/blob/9e40c7dc4ad5b5e9f85cb434a14b7826e954db5b/Readme%20and%20Img/Images/Register.png" alt="RegiterPage">

LoginPage
<img src="https://github.com/revanthvezzu-sketch/Pizza-Store-Capstone-Project/blob/9e40c7dc4ad5b5e9f85cb434a14b7826e954db5b/Readme%20and%20Img/Images/Login.png" alt="LoginPage">

User-DashBoard
<img src="https://github.com/revanthvezzu-sketch/Pizza-Store-Capstone-Project/blob/9e40c7dc4ad5b5e9f85cb434a14b7826e954db5b/Readme%20and%20Img/Images/UserDashboard.png" alt="User-DashBoard">

Admin-DashBoard
<img src="https://github.com/revanthvezzu-sketch/Pizza-Store-Capstone-Project/blob/9e40c7dc4ad5b5e9f85cb434a14b7826e954db5b/Readme%20and%20Img/Images/AdminDashboard.png" alt = "User-Dashbord">

Revenue-DashBoard(inside Admin)
<img src = "https://github.com/revanthvezzu-sketch/Pizza-Store-Capstone-Project/blob/9e40c7dc4ad5b5e9f85cb434a14b7826e954db5b/Readme%20and%20Img/Images/RevenueDasboard.png" alt = "Revenue - DashBoard">

<p>
<h3>Database Images</h3>

User
<img src="https://github.com/revanthvezzu-sketch/Pizza-Store-Capstone-Project/blob/51dd9f6c9dda551f8d92192c7b6166e730e77b82/Database-img/userdb.png" alt="UserDb">

Order
<Img src="https://github.com/revanthvezzu-sketch/Pizza-Store-Capstone-Project/blob/51dd9f6c9dda551f8d92192c7b6166e730e77b82/Database-img/orderdb.png" alt="OrderDb" >

Bill
<img src="https://github.com/revanthvezzu-sketch/Pizza-Store-Capstone-Project/blob/51dd9f6c9dda551f8d92192c7b6166e730e77b82/Database-img/billdb.png" alt="BillDb">
</p>
